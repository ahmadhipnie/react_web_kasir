  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const transactionData = {
        ...
        total_payment: total,
        money_received: paymentMethod === 'cash' ? parseFloat(cashReceived) : total,
        change_money: paymentMethod === 'cash' ? change : 0,
        ...
        <div className="menu-grid">
          {filteredFoods.length > 0 ? (
            filteredFoods.map((food) => (
              <div 
                key={food.id} 
                className={`menu-item ${food.stock <= 0 ? 'out-of-stock' : ''} ${cart.find(i => i.id === food.id) ? 'selected' : ''}`}
                onClick={() => food.stock > 0 && addToCart(food)}
              >
                ...
                <div className="menu-item-info">
                  <h4 className="menu-item-name">{food.food_name}</h4>
                  <p className="menu-item-price">{formatCurrency(food.price)}</p>
          ...
          {lastTransaction && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                ...
                <strong>{lastTransaction.transaction_code}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ color: 'var(--gray-500)' }}>Total Payment</span>
                <strong>{formatCurrency(lastTransaction.total_payment)}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--gray-500)' }}>Method</span>
                <strong style={{ textTransform: 'capitalize' }}>{lastTransaction.payment_method}</strong>
              </div>
            </div>
          )}
